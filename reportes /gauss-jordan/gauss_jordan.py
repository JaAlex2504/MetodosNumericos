    ...
    try:
        ...
        if n < 2:
            messagebox.showwarning(
                "Atención", "Las ecuaciones deben tener al menos 2 incógnitas."
            )
            return
        ...
        tk.Label(frame_tabla, text="Resultado", font=("Arial", 9, "bold")).grid(
            row=0, column=n
        )
...
def resolver_gauss_jordan():
    try:
        ...
        messagebox.showerror(
            "Error", "Asegúrate de llenar todos los campos con números."
        )
...
tk.Button(controles, text="Generar matriz", command=generar_interfaz_matriz).grid(
